...
function redirect(request: Request, result: string, entity?: string) { const url = new URL(`/master-data?${result}=1`, request.url); if (entity) url.searchParams.set("section", entity); return NextResponse.redirect(url, 303); }
...
export async function POST(request: Request) {
  ...
  if (intent === "delete") {
    ...
    return redirect(request, "deleted", entity);
  }
  const active = form.get("isActive") === "on";
  const name = nameSchema.safeParse(form.get("name"));
  if (!name.success) return redirect(request, "error", entity);
  if (entity === "PROJECT") {
  ...
  if (entity === "CATEGORY") {
    ...
    if (scope && !scope.success) return redirect(request, "error", entity);
  ...
  if (entity === "TASK") {
    ...
    if (!scope.success || (projectId && !z.string().uuid().safeParse(projectId).success)) return redirect(request, "error", entity);
  ...
  return redirect(request, intent === "create" ? "created" : "updated", entity);
}
