...
export async function POST(request: Request) {
  ...
  if (id && !(await rows<any>("SELECT id FROM "+targetTable+" WHERE id=?",[id])).length) return redirect("error",entity);
  if (intent === "delete") {
    const table = entity === "PROJECT" ? "Project" : entity === "CATEGORY" ? "Category" : "Task";
    if(await masterRecordInUse(entity,id!))return redirect("error",entity);
  ...
  if (entity === "CATEGORY") {
    ...
    if(intent==="update") { const [existing]=await rows<any>("SELECT kind FROM Category WHERE id=?",[id]); if(await masterRecordInUse("CATEGORY",id!) && existing?.kind!==kind.data)return redirect("error",entity); }
  ...
  if (entity === "TASK") {
    ...
    if (projectId && !(await rows<any>("SELECT id FROM Project WHERE id=?",[projectId])).length) return redirect("error",entity);
    if(intent === "update") {
      ...
      if(existing?.owner !== viewer) {
        ...
        return redirect("updated",entity);
      }
    }
