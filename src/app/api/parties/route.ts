...
import { transaction, masterRecordInUse } from "@/lib/db";
...
export async function POST(request:Request){
  ...
  try{await transaction(async c=>{
    if(d.id){
      ...
      const used=await masterRecordInUse("PARTY",d.id);
      if(used&&(found[0].kind!==d.kind||Boolean(found[0].isCash)!==isCash))throw new Error("history");
    ...
    }else await c.execute("INSERT INTO Party (owner,id,name,kind,contactNo,isCash,isActive) VALUES (?,?,?,?,?,?,?)",[viewer,crypto.randomUUID(),d.name,d.kind,d.contactNo||null,isCash,active]);
