...
function redirect(result: string, returnTo?: string) { return relativeRedirect(returnTo==="/master-data/accounts"?`${returnTo}?${result}=1`:`/master-data?${result}=1&section=ACCOUNT`); }
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (form.get("intent") === "delete") {
    ...
    return redirect("deleted",returnTo);
  }
