...
const money = (n: number) => new Intl.NumberFormat("en-LK", { style: "currency", currency: "LKR", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);
    ...
    <div className="ac-layout">
      ...
      <div className="ac-detail">{!account ? <section className="panel empty"><h2>{accounts.length ? "Choose an account" : "Your accounts start here"}</h2><p>Select an account to see its complete statement and balance trends.</p><Link className="button primary" href="/master-data?section=ACCOUNT">Set up accounts</Link></section> : <>
        ...
        {invalidDates ? <p role="alert" className="ac-alert">The end date must be on or after the start date.</p> : <>
          <div className="ac-metrics ac-period-metrics"><Metric title="Opening balance" value={summary.opening} note="Before the first entry in this period" /><Metric title={debt ? "Debt increases" : "Money in"} value={summary.increases} note="Includes transfers and opening entries" /><Metric title={debt ? "Debt reductions" : "Money out"} value={summary.decreases} note="All decreases in the account balance" /><Metric title="Closing balance" value={summary.closing} note={`${summary.entries.length} ledger entries in period`} /></div>
          <div className="ac-charts"><section className="panel"><p className="eyebrow">Balance journey</p><h2>{debt ? "Debt over time" : "Balance over time"}</h2><p className="ac-caption">Daily closing balances · hover or focus a point for details.</p><BalanceChart days={summary.days} /></section><section className="panel"><p className="eyebrow">Monthly movement</p><h2>{debt ? "Borrowing & repayments" : "Inflow & outflow"}</h2><p className="ac-caption">Latest 6 active months in period. Transfers included; opening balances excluded.</p><div className="ac-legend"><span>● {debt ? "Debt added" : "In"}</span><span>● {debt ? "Debt reduced" : "Out"}</span></div><MovementChart months={summary.months.slice(-6)} /></section></div>
          <section className="panel ac-spending"><div><p className="eyebrow">Spending focus</p><h2>Where spending goes</h2><p className="ac-caption">Expense entries only. Transfers and debt repayments are excluded.</p><strong>{money(summary.expenses)}</strong><p className="ac-caption">Recorded income: {money(summary.income)}</p></div><div className="ac-category-bars">{summary.categories.slice(0, 5).map(c => <div key={c.name}><span>{c.name}<b>{money(c.amount)} · {summary.expenses ? (c.amount / summary.expenses * 100).toFixed(0) : 0}%</b></span><progress max={summary.expenses || 1} value={c.amount} aria-label={`${c.name} spending`} /></div>)}{!summary.categories.length && <p className="muted">No expense entries in this period.</p>}{summary.categories.length > 5 && <small>Top 5 of {summary.categories.length} categories</small>}</div></section>
...
function Metric({ title, value, note }: { title: string; value: number; note: string }) { return <article className="ac-metric"><p>{title}</p><strong>{money(value)}</strong><small>{note}</small></article>; }

function BalanceChart({ days }: { days: { date: string; balance: number }[] }) {
  ...
  return <div className="ac-balance-chart"><p aria-live="polite">{day(selected.date)} <b>{money(selected.balance)}</b></p><svg viewBox="0 0 500 190" role="img" aria-label={`Daily closing balance from ${day(days[0].date)} to ${day(days.at(-1)!.date)}`}><line x1="18" x2="482" y1={y(0)} y2={y(0)} stroke="#d5dfda" strokeDasharray="4 4" /><polyline points={days.map((d, i) => `${x(i)},${y(d.balance)}`).join(" ")} fill="none" stroke="#087e68" strokeWidth="2.5" />{days.map((d, i) => <circle key={d.date} cx={x(i)} cy={y(d.balance)} r={i === active ? 6 : 3.5} fill="#087e68" tabIndex={0} aria-label={`${day(d.date)}: ${money(d.balance)}`} onFocus={() => setActive(i)} onMouseEnter={() => setActive(i)}><title>{`${day(d.date)}: ${money(d.balance)}`}</title></circle>)}</svg><div className="ac-chart-axis"><span>{day(days[0].date)}</span><span>{day(days.at(-1)!.date)}</span></div></div>;
}
