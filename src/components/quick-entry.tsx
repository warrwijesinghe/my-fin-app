...
type Option = { id: string; name: string; projectId?: string | null; kind?:string; householdExpenseClass?:string };
...
const entryTypes: { value: EntryType; label: string; detail: string }[] = [
  ...
  { value: "TRANSFER", label: "Transfer / asset purchase", detail: "Move money between accounts or into a fixed asset" },
...
function EntryForm({ type, accounts, projects, tasks, categories, today, items, initialOwner="ME", initialHousehold=false, parties=[], draft, initialScope="PERSONAL" }: EntryProps & { type: EntryType }) {
  ...
  const [expenseKind,setExpenseKind]=useState<""|"HOUSEHOLD"|"PERSONAL"|"BUSINESS">("");
  ...
  const paymentAccounts=eligibleAccounts.filter(account=>account.type!=="FIXED_ASSET");
  ...
  const isExpense = type === "EXPENSE" || type === "ACCRUED_EXPENSE";
  return <form className="form-grid quick-entry-form" action="/api/transactions" method="post">
      ...
      {(type==="EXPENSE"||type==="ACCRUED_EXPENSE")&&<div className="payer-account"><label>Who will pay?<select name="spentBy" value={spentBy} onChange={e=>{setSpentBy(e.target.value as "ME"|"WIFE");setAccountId("");}}><option value="ME">WARR Wijesinghe</option><option value="WIFE">JAD Buddhika</option></select></label>{needsAccount&&<label>Paid from<select name="accountId" value={accountId} onChange={e=>setAccountId(e.target.value)} required><option value="" disabled>Select account</option>{paymentAccounts.map((account) => <option key={account.id} value={account.id}>{account.name} ({account.type.replaceAll("_", " ")})</option>)}</select></label>}<input name="returnTo" type="hidden" value={initialHousehold?"household":""}/></div>}
      {isExpense&&<div className="expense-kind"><span>Expense type</span><label className="expense-option"><input type="radio" name="expenseKind" value="HOUSEHOLD" checked={expenseKind==="HOUSEHOLD"} required onChange={()=>{setExpenseKind("HOUSEHOLD");setScope("PERSONAL");setTaxScope("PERSONAL");}}/>Household expense</label><label className="expense-option"><input type="radio" name="expenseKind" value="BUSINESS" checked={expenseKind==="BUSINESS"} onChange={()=>{setExpenseKind("BUSINESS");setScope("BUSINESS");setTaxScope("BUSINESS");}}/>Business expense</label></div>}
      {needsAccount && type!=="EXPENSE" && <label>{type === "INCOME" ? "Received into" : type === "TRANSFER" || type === "DEBT_PAYMENT" ? "Pay from" : "Paid from"}<select name="accountId" value={accountId} onChange={e=>setAccountId(e.target.value)} required><option value="" disabled>Select account</option>{(type==="TRANSFER"?eligibleAccounts:paymentAccounts).map((account) => <option key={account.id} value={account.id}>{account.name} ({account.type.replaceAll("_", " ")})</option>)}</select></label>}
      {needsAccount&&!paymentAccounts.length&&<p className="span-2">No payment account available. <a href="/master-data?section=ACCOUNT">Create an account in Master Data.</a></p>}
      {needsDestination && <label>{type === "DEBT_PAYMENT" ? "Pay this debt" : "Transfer to / buy asset"}<select name="destinationAccountId" defaultValue="" required><option value="" disabled>Select account</option>{(type === "DEBT_PAYMENT" ? debtAccounts : eligibleAccounts).map((account) => <option key={account.id} value={account.id}>{account.name} ({account.type.replaceAll("_", " ")})</option>)}</select>{type==="TRANSFER"&&<small>Choose a Fixed Asset account for land purchase, documentation, valuation, and other direct acquisition costs.</small>}</label>}
      ...
      {supportsClassification&&<div className="span-2 form-grid entry-more-grid">
        {isExpense ? (expenseKind ? <><input name="scope" type="hidden" value={expenseKind==="BUSINESS"?"BUSINESS":"PERSONAL"}/><div className="actual-tax"><label>Actual label<select name="scope" value={expenseKind==="BUSINESS"?"BUSINESS":"PERSONAL"} disabled><option>{expenseKind==="BUSINESS"?"Business":"Personal"}</option></select></label>{expenseKind==="BUSINESS" ? <><input name="taxScope" type="hidden" value="BUSINESS"/><label>Tax label<select name="taxScope" value="BUSINESS" disabled><option>Business</option></select></label></> : <><input name="taxScope" type="hidden" value="PERSONAL"/><label>Tax label<select name="taxScope" value="PERSONAL" disabled><option>Personal</option></select></label></>}</div></> : <div className="actual-tax"><label>Actual label<select name="scope" value="" disabled><option>Choose expense type first</option></select></label><label>Tax label<select name="taxScope" value="" disabled><option>Choose expense type first</option></select></label></div>) : <div className="actual-tax"><label>Actual label<select name="scope" value={scope} onChange={(event) => setScope(event.target.value)}><option value="PERSONAL">Personal</option><option value="BUSINESS">Business</option></select></label><label>Tax label<select name="taxScope" value={taxScope ?? scope} onChange={(event) => setTaxScope(event.target.value)}><option value="PERSONAL">Personal</option><option value="BUSINESS">Business</option></select></label></div>}
