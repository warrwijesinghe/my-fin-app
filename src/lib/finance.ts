...
export async function getDashboardData(scope?: MoneyScope) {
  ...
  const date=start.toISOString().slice(0,10); const viewer=await currentOwner(); const [totals,pendingRows,accrualRows,receivableRows,loanRows,cashReceivableRows]=await Promise.all([rows<RowDataPacket>(`SELECT type,COALESCE(SUM(amount),0) amount FROM \`FinancialTransaction\` WHERE status='POSTED' AND transactionDate>=?${where} GROUP BY type`,[date,...values]),rows<RowDataPacket>("SELECT COUNT(*) count FROM `FinancialTransaction` WHERE status='PENDING_REVIEW'"),rows<RowDataPacket>(`SELECT COALESCE(SUM(amount-paidAmount),0) amount FROM \`AccruedExpense\` WHERE status IN ('OPEN','PARTIALLY_PAID')${where}`,values),rows<RowDataPacket>(`SELECT COALESCE(SUM(GREATEST(balance,0)),0) amount FROM CreditOutstanding WHERE type='INCOME'${where}`,values),rows<RowDataPacket>("SELECT lender,borrower,amount,repaidAmount FROM FamilyLoan WHERE status IN ('OPEN','PARTIALLY_PAID')"),rows<RowDataPacket>("SELECT COALESCE(SUM(GREATEST(amount-receivedAmount,0)),0) amount FROM Receivable WHERE status IN ('OPEN','PARTIALLY_PAID')")]); const [pending]=pendingRows,[accrual]=accrualRows,[receivables]=receivableRows,[cashReceivables]=cashReceivableRows;
  const outstandingReceivables=Number(receivables?.amount??0), cashAdvanceReceivables=Number(cashReceivables?.amount??0), totalReceivables=outstandingReceivables+cashAdvanceReceivables;
  const familyReceivable=loanRows.filter(loan=>loan.lender===viewer).reduce((sum,loan)=>sum+Math.max(0,Number(loan.amount)-Number(loan.repaidAmount)),0), familyPayable=loanRows.filter(loan=>loan.borrower===viewer).reduce((sum,loan)=>sum+Math.max(0,Number(loan.amount)-Number(loan.repaidAmount)),0);
  ...
  return {accounts:filtered,assets,availableCash,debt,outstandingAccruals,outstandingReceivables,cashAdvanceReceivables,totalReceivables,familyReceivable,familyPayable,overallPosition:assets+totalReceivables+familyReceivable-debt-outstandingAccruals-familyPayable,income,expenses,netMovement:income-expenses,pending:Number(pending?.count??0)};
}
