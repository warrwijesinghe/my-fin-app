...
export async function getDashboardData(scope?: MoneyScope) {
  ...
  const [receivables]=await rows<RowDataPacket>(`SELECT COALESCE(SUM(GREATEST(balance,0)),0) amount FROM CreditOutstanding WHERE owner='ME' AND type='INCOME'${where}`,values);
